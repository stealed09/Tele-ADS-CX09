...
async def gm_add_folder_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        await query.edit_message_text(
            ...
            "Format: `t.me/addlist/XXXXXXXXX`\n\n"
            "The bot will fetch all groups in the folder and let you configure them.",
            parse_mode='Markdown',
            reply_markup=menu_ui.back_button()
        )
    except BadRequest:
...
async def process_folder_link(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await wait_msg.edit_text(
        ...
        f"Now send your message and interval in this format:\n\n"
        f"`<interval_minutes> <your message text>`\n\n"
...
async def gm_confirm_selection_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        await query.edit_message_text(
            ...
            f"Now send your message + interval:\n\n"
            f"`<interval_minutes> <your message>`\n\n"
...
async def add_group_message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        await query.edit_message_text(
            ...
            "Interval = minutes between each send",
            parse_mode='Markdown',
            reply_markup=menu_ui.back_button()
        )
    except BadRequest:
...
async def view_group_messages_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for msg in messages_list:
        ...
        keyboard_rows.append([
            InlineKeyboardButton(f"🗑 Delete #{msg['id']}", callback_data=f"del_group_msg_{msg['id']}")
        ])
...
def main():
    ...
    # Group Messages — Add Single / View / Delete
