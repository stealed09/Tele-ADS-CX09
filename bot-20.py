...
async def gm_set_message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Prompt user to set message and interval."""
    query = update.callback_query
    await query.answer()
    try:
        await query.edit_message_text(
...
async def process_broadcast_msg(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "\u2705 *Broadcast configured!*\n\n\u23f1 Interval: every *" + str(interval) + "* min\n\U0001f4dd Message: `" + message[:80] + "`\n\nNow turn it *ON* from the menu.",
        parse_mode='Markdown',
        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("\U0001f4e2 Broadcast Menu", callback_data="group_messages")]])
    )
...
async def gm_toggle_broadcast_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        await query.edit_message_text(text, reply_markup=menu_ui.group_messages_menu(), parse_mode='Markdown')
    except BadRequest:
        pass
