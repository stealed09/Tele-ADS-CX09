...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_admin(user.id):
        ...
        if not account:
            ...
            await update.message.reply_text(
                ...
                reply_markup=menu_ui.main_menu(is_admin=True, broadcast_active=broadcast_active, broadcast_active=False),
                parse_mode='Markdown'
            )
        return
    ...
    if access_status == 'approved':
        ...
        if not account:
            ...
            await update.message.reply_text(
                f"👋 **Welcome Back!**\n\n✅ Active: {account['phone']}\n\nChoose action:",
                reply_markup=menu_ui.main_menu(is_admin=False, broadcast_active=broadcast_active, broadcast_active=False),
                parse_mode='Markdown'
            )
