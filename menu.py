...
class MenuUI:
    ...
    @staticmethod
    def group_messages_menu(any_active: bool = True):
        """Group auto messages main menu.
        any_active: True if at least one group message is currently active.
        """
        master_label = "🔴 STOP ALL (Master OFF)" if any_active else "🟢 START ALL (Master ON)"
        master_cb = "gm_master_off" if any_active else "gm_master_on"
        keyboard = [
            [InlineKeyboardButton(master_label, callback_data=master_cb)],
